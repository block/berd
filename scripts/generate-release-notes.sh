...
**Full Changelog**: https://github.com/squareup/goose-internal/compare/${FROM_REF}...${COMPARE_TO}"
...
EXISTING_BODY="$(gh release view "$TO_REF" --repo squareup/goose-internal --json body -q .body 2>/dev/null || true)"
...
You are publishing release notes for the squareup/goose-internal repository.

Use the \`gh\` CLI to set the notes on the GitHub release for tag \`${TO_REF}\`:

1. Verify the release exists: \`gh release view ${TO_REF} --repo squareup/goose-internal\`.
...
2. Update only the release notes body, leaving title, tag, target, and assets
   ...
   \`gh release edit ${TO_REF} --repo squareup/goose-internal --notes-file <tempfile>\`.
