// Scaffolds a new goosectl command: `just new-command <noun> <verb>`.
//
...
// (`cargo test -p goosectl`) FAILS until the TODO prose is written.
//
...
// do those by hand per .agents/skills/goosectl-new-command/SKILL.md.
...
function die(message) {
  console.error(`new-goosectl-command: ${message}`);
...
if (!/^[a-z][a-z0-9]*$/.test(verb)) {
  die(
    `verb "${verb}" must be a single lowercase word; multi-word verbs need ` +
      ...
      ".agents/skills/goosectl-new-command/SKILL.md",
  );
}
if (verb === "help") {
...
const surfacePath = path.join(
  repoRoot,
  "src-tauri/crates/goosectl/cli-surface.json",
);
...
if (nounSpec.verbs[verb]) {
  die(`\`goosectl ${noun} ${verb}\` already exists; pick a different verb`);
}
...
try {
  execFileSync("pnpm", ["generate:goosectl-contract", "--check"], {
    cwd: repoRoot,
    stdio: "pipe",
  });
} catch {
  die(
    "the contract generator cannot run cleanly (`pnpm " +
      "generate:goosectl-contract --check` failed). Fix the generator error; " +
...
const modulePath = path.join(
  repoRoot,
  `src/features/goosectl/commands/impl/${moduleBase}.ts`,
);
...
export const ${commandName} = defineCommand({
  ...
  visibility: "none",
  destructive: false,
  // TODO(scaffold): one-line summary for \`goosectl ${noun} --help\`'s verb
  // list.
  summary: "TODO",
...
const registryPath = path.join(
  repoRoot,
  "src/features/goosectl/commands/registry.ts",
);
...
const testPath = path.join(
  repoRoot,
  "src/features/goosectl/__tests__/commands/commands.test.ts",
);
...
const mainPath = path.join(repoRoot, "src-tauri/crates/goosectl/src/main.rs");
...
execFileSync("pnpm", ["generate:goosectl-contract"], {
  cwd: repoRoot,
  stdio: "inherit",
});

console.log(`
Scaffolded \`goosectl ${noun} ${verb}\` (${group}.${verb}). Authored work left:

  1. ${path.relative(repoRoot, modulePath)}
     — schema (bounds! .describe() every field), summary, description,
     helpFooter, precheck, execute. \`cargo test -p goosectl\` FAILS until
     the TODO prose is real.
  2. src/features/goosectl/__tests__/commands/commands.test.ts
  ...
  3. The verb inventories: the group description and cli.about in
     ...
     is EXPECTED_TOP_LEVEL_HELP in src-tauri/crates/goosectl/src/main.rs,
     refreshed via dump_rendered_help_for_pin_update), and — if the
     one-paragraph overview changes — distro/skills/goosectl/SKILL.md
  4. Re-run \`pnpm generate:goosectl-contract\` after schema edits

Reviewer checklist (from .agents/skills/goosectl-new-command/SKILL.md):
