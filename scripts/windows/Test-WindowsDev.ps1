...
try {
    ...
    Assert-Equal "Windows externalBin excludes berd-memory-mcp" ($windowsExternalBin -contains "binaries/berd-memory-mcp") $false
    ...
    # Unsupported Windows bundles must remain memory-free after overlays.
    Assert-Equal "merged Windows externalBin excludes berd-memory-mcp" ($mergedExternalBin -contains "binaries/berd-memory-mcp") $false
