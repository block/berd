...
#[cfg(feature = "apps-e2e-test")]
struct AppsHttpsMockServer {
    address: String,
    requests: Arc<Mutex<Vec<common::RecordedRequest>>>,
    handle: Option<thread::JoinHandle<()>>,
}

#[cfg(feature = "apps-e2e-test")]
impl AppsHttpsMockServer {
    fn start(responses: Vec<MockResponse>) -> Self {
        let server = tiny_http::Server::https(
            "127.0.0.1:0",
            tiny_http::SslConfig {
                certificate: include_bytes!("fixtures/apps-e2e-server.pem").to_vec(),
                private_key: include_bytes!("fixtures/apps-e2e-server-key.pem").to_vec(),
            },
        )
        .expect("bind Apps HTTPS mock server");
        let address = server
            .server_addr()
            .to_ip()
            .expect("Apps HTTPS mock IP address")
            .to_string();
        ...
        let handle = thread::spawn(move || {
            for response in responses {
                let mut request = server
                    ...
                    .expect("receive Apps HTTPS mock request")
                    .expect("Apps HTTPS mock request before timeout");
                ...
                request
                    ...
                    .expect("read Apps HTTPS mock request");
                let body = if headers
                    .get("content-type")
                    .is_some_and(|value| value.starts_with("application/json"))
                {
                    serde_json::from_slice(&body_bytes).expect("parse Apps HTTPS JSON request")
                } else {
                    Value::Null
                };
                thread_requests
                    .lock()
                    .expect("lock Apps HTTPS requests")
                    .push(common::RecordedRequest {
                        method,
                        path,
                        headers,
                        body,
                ...
                for (name, value) in response.headers {
                    reply.add_header(
                        tiny_http::Header::from_bytes(name, value)
                            .expect("build Apps HTTPS response header"),
                    );
                }
                request
                    .respond(reply)
                    .expect("respond to Apps HTTPS request");
            }
        });
        Self {
            address,
    ...
    fn finish(mut self) -> Vec<common::RecordedRequest> {
        self.handle
            .take()
            .expect("Apps HTTPS mock server handle")
            .join()
            .expect("join Apps HTTPS mock server");
        self.requests
            .lock()
            .expect("lock Apps HTTPS requests")
            .clone()
...
#[cfg(feature = "apps-e2e-test")]
fn configured_apps_command(
    auth_server: &MockServer,
    control_plane: &AppsHttpsMockServer,
    temp: &Path,
    credential: &str,
) -> std::process::Command {
    let bb_home = temp.join("bb-home");
    ...
    command
        ...
        .env("BB_APPS_E2E_RESOLVE_ADDR", &control_plane.address);
    command
}
...
fn bb_apps_contract_verifies_and_uses_the_same_session() {
    ...
    let auth_server = MockServer::start(vec![apps_auth_me_response()]);
    let control_plane = AppsHttpsMockServer::start(vec![MockResponse::json(contract.clone())]);
    let temp = temp_test_dir("bb-apps-contract-success");

    let output = configured_apps_command(&auth_server, &control_plane, &temp, credential)
        .args([
            "apps",
            "contract",
            "--base-url",
    ...
    assert_apps_auth_request(&auth_requests[0], credential);
    assert_eq!(control_plane_requests.len(), 1);
    assert_apps_control_plane_request(
        &control_plane_requests[0],
        "GET",
        "/v1/agent/contract",
        credential,
        "0.2.0",
...
fn bb_apps_create_runs_plan_and_initialize_end_to_end() {
    ...
    let auth_server = MockServer::start(vec![apps_auth_me_response()]);
    let control_plane = AppsHttpsMockServer::start(vec![
...
fn bb_apps_create_skips_initialize_when_plan_does_not_request_it() {
    ...
    let auth_server = MockServer::start(vec![apps_auth_me_response()]);
    let control_plane = AppsHttpsMockServer::start(vec![MockResponse::json(plan.clone())]);
...
fn bb_apps_deploy_uploads_multipart_artifact_end_to_end() {
    ...
    let auth_server = MockServer::start(vec![apps_auth_me_response()]);
    let control_plane = AppsHttpsMockServer::start(vec![MockResponse::json(deployed.clone())]);
    let temp = temp_test_dir("bb-apps-deploy-success");
    let artifact_path = temp.join("prepared-app.tar.gz");
    fs::write(&artifact_path, "test-hotpod-artifact-marker").expect("write deploy artifact");
