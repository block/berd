...
import {
  isGoosectlCrossSessionQueuedMessage,
  sendPromptToExistingSessionInBackground,
} from "@/features/goosectl/commands/runtime/sessionSend";
...
function drainQueuedMessage(sessionId: string): void {
  ...
  void sendPromptToExistingSessionInBackground(sessionId, queuedMessage.text)
    ...
    .catch((error) => {
      console.error(
        `[goosectl-queue] failed to send queued prompt for session ${sessionId}`,
        error,
      );
    })
    .finally(() => {
...
export function useGoosectlQueuedMessageDrain(): void {
  useEffect(() => {
    ...
    return useChatStore.subscribe((state, previousState) => {
      for (const sessionId of Object.keys(state.queuedMessageBySession)) {
        ...
        if (!isGoosectlCrossSessionQueuedMessage(queuedMessage)) {
          continue;
        }
