...
/**
 * Builders for the goosectl contract artifacts
 * (src-tauri/crates/goosectl/api-surface.json and cli-surface.json).
 *
 ...
 *   prose) the bundled goosectl binary hand-projects from.
 *
 * `scripts/generate-goosectl-contract.mjs` writes both to disk; the goosectl
 ...
 *  Mirror of `PROTOCOL_VERSION` in both discovery.rs copies (a goosectl
...
type VerbSpec = {
  ...
  action: string;
  /** One-line summary in `goosectl <noun> --help`'s verb list. */
  about: string;
...
const API_COMMENT =
  "GENERATED FILE — do not hand-edit; run `pnpm generate:goosectl-contract`. " +
  "Client-neutral wire surface of the Goose desktop app's control API: " +
  'POST /v1/call {"command": "<group>", "args": {"action": "<action>", ' +
  "...fields}} against the loopback endpoint in the goosectl discovery file. " +
  ...
  "(src/features/goosectl/__tests__/apiSurface.test.ts).";
...
const CLI_COMMENT =
  ...
  "command's summary/helpFooter (src/features/goosectl/commands/registry.ts " +
  "and impl/*.ts). goosectl embeds this file plus api-surface.json and " +
  "builds its clap tree from them at startup (tree.rs); vitest asserts " +
  "freshness (src/features/goosectl/__tests__/cliSurface.test.ts).";
...
function describeField(
  ...
  throw new Error(
    `${group}.${action}.${name}: unsupported zod type ` +
      ...
      "the goosectl crate's tree.rs/wire.rs to build a flag for it), then " +
      "regenerate with `pnpm generate:goosectl-contract`",
  );
}
