...
/**
 * The goosectl command surface: one group per domain, dispatching on an
 ...
 * test-only deadline fallback below). The goosectl CLI's noun/verb surface
 * mirrors these groups/actions via src-tauri/crates/goosectl/cli-surface.json,
 * which is generated from this registry's `cli` metadata
 * (`pnpm generate:goosectl-contract`) and asserted fresh on both sides.
 *
 * `cli.noun` is the CLI's (singular) noun for the group; `cli.about` is the
 * noun's one-line entry in `goosectl --help`'s command list; verbs default to
