...
export async function loadAllSessionsForGoosectl(): Promise<void> {
  try {
    await loadSessionsForGoosectlUntil(() => false, { exhaust: true });
  } catch (error) {
...
export async function loadSessionForGoosectl(sessionId: string): Promise<void> {
  let found = false;
  try {
    found = await loadSessionsForGoosectlUntil(
...
async function loadSessionsForGoosectlUntil(
