...
export const sendSessionCommand = defineCommand({
  ...
  description:
    ...
    "by Goose from another session. Running sessions are refused by default; use " +
...
Result:
  ...
  execute: async (args): Promise<SendSessionResult> => {
    const [
      { loadSessionForGoosectl, requireSession },
      {
      ...
      switch (args.if_running) {
        ...
        case "queue":
          ...
          chatStore.enqueueMessage(args.session_id, {
            text: args.prompt,
            sendOptions: goosectlCrossSessionSendOptions(),
          });
