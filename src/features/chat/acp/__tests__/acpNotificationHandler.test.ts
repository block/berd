...
vi.mock("@/features/terminal/lib/acpTerminalManager", () => ({
  appendExternalAcpTerminalOutput: vi.fn(() => Promise.resolve()),
  finishExternalAcpTerminal: vi.fn(() => Promise.resolve()),
  registerExternalAcpTerminal: vi.fn(() => Promise.resolve()),
  requestOpenAcpTerminal: vi.fn(),
}));
...
describe("acpNotificationHandler", () => {
  ...
  it("preserves ACP terminal references on tool requests", async () => {
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
        sessionUpdate: "tool_call",
        ...
        content: [{ type: "terminal", terminalId: "agent-terminal-1" }],
      },
    });

    const message = useChatStore
      .getState()
      .messagesBySession["acp-session"]?.at(-1);
    expect(
      message?.content.find((content) => content.type === "toolRequest"),
    ).toMatchObject({
      type: "toolRequest",
      terminalId: "agent-terminal-1",
    });
    expect(registerExternalAcpTerminal).toHaveBeenCalledWith({
      sessionId: "acp-session",
      terminalId: "agent-terminal-1",
      cwd: "~",
      title: "pnpm dev",
    ...
    const messageBeforeOutput = message;
    const requestBeforeOutput = message?.content.find(
      ...
      update: {
        ...
        _meta: {
          terminal_output_delta: {
            ...
            data: "Serving on port 8765\n",
          },
        },
      },
    } as never);
    const messageAfterOutput = useChatStore
      .getState()
      .messagesBySession["acp-session"]?.at(-1);
    expect(messageAfterOutput).toBe(messageBeforeOutput);
    expect(
      messageAfterOutput?.content.find(
        (content) => content.type === "toolRequest",
      ),
    ).toBe(requestBeforeOutput);
    expect(appendExternalAcpTerminalOutput).toHaveBeenCalledWith(
      "acp-session",
      "agent-terminal-1",
      "Serving on port 8765\n",
    );
    ...
    expect(finishExternalAcpTerminal).toHaveBeenCalledWith(
      "acp-session",
      "agent-terminal-1",
  ...
  it("correlates a late terminal id with its distinct tool call id", async () => {
    vi.useFakeTimers();
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
    ...
    await handleSessionNotification({
      ...
      update: {
        sessionUpdate: "tool_call_update",
        toolCallId: "tool-call-1",
        content: [{ type: "terminal", terminalId: "terminal-99" }],
      },
    } as never);
    ...
    expect(
      useChatStore
        .getState()
        .messagesBySession["acp-session"]?.at(-1)
        ?.content.find((content) => content.type === "toolRequest"),
    ).toMatchObject({ id: "tool-call-1", terminalId: "terminal-99" });
    await vi.advanceTimersByTimeAsync(5_000);
    expect(requestOpenAcpTerminal).toHaveBeenCalledWith(
      "acp-session",
      "terminal-99",
      { automatic: true },
    );
  ...
  it("does not auto-open a terminal that exits during the grace period", async () => {
    vi.useFakeTimers();
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
        sessionUpdate: "tool_call",
        toolCallId: "short-command",
        title: "Short command",
        status: "in_progress",
        rawInput: {},
        content: [{ type: "terminal", terminalId: "short-terminal" }],
      },
    } as never);
    ...
    await vi.advanceTimersByTimeAsync(5_000);
    expect(requestOpenAcpTerminal).not.toHaveBeenCalled();
    vi.useRealTimers();
  });

  it("does not schedule automatic open when exit precedes registration", async () => {
    vi.useFakeTimers();
    let finishRegistration: (() => void) | undefined;
    vi.mocked(registerExternalAcpTerminal).mockReturnValueOnce(
    ...
    await handleSessionNotification({
      ...
      update: {
        sessionUpdate: "tool_call",
        toolCallId: "racing-command",
        title: "Racing command",
        status: "in_progress",
        rawInput: {},
        content: [{ type: "terminal", terminalId: "racing-terminal" }],
      },
    } as never);
    await handleSessionNotification({
      sessionId: "acp-session",
  ...
  it("does not infer a terminal from an arbitrary command-shaped tool input", async () => {
    vi.useFakeTimers();
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
        sessionUpdate: "tool_call",
        toolCallId: "deploy-tool-1",
        title: "Deploy application",
        status: "in_progress",
        kind: "other",
        rawInput: { command: "deploy" },
      },
    } as never);
    ...
    expect(registerExternalAcpTerminal).not.toHaveBeenCalled();
    await vi.advanceTimersByTimeAsync(5_000);
    expect(requestOpenAcpTerminal).not.toHaveBeenCalled();
    vi.useRealTimers();
  });

  it("recovers a Codex terminal id when relay kind and metadata are stripped", async () => {
    vi.useFakeTimers();
    registerPreparedSession("acp-session", "codex-acp", "/Users/test");
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
        sessionUpdate: "tool_call",
        toolCallId: "codex-command-1",
        title: "python3 -m http.server 8766",
        status: "in_progress",
        rawInput: {
          command: "python3 -m http.server 8766",
          cwd: "/Users/test/goose artifacts",
        },
      },
    } as never);
  ...
  it("recovers a replayed terminal when live output resumes without auto-opening it", async () => {
    vi.useFakeTimers();
    useChatStore.setState({ loadingSessionIds: new Set(["acp-session"]) });
    await handleSessionNotification({
      sessionId: "acp-session",
      update: {
        sessionUpdate: "tool_call",
        toolCallId: "replayed-tool",
        title: "Replayed server",
        rawInput: { cwd: "/repo" },
        content: [{ type: "terminal", terminalId: "replayed-terminal" }],
      },
    } as never);
    useChatStore.setState({ loadingSessionIds: new Set() });
    ...
    expect(registerExternalAcpTerminal).toHaveBeenCalledOnce();
    expect(registerExternalAcpTerminal).toHaveBeenCalledWith({
      sessionId: "acp-session",
      terminalId: "replayed-terminal",
      cwd: "/repo",
      title: "Replayed server",
    });
    expect(appendExternalAcpTerminalOutput).toHaveBeenCalledWith(
