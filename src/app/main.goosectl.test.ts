...
describe("main entrypoint goosectl bridge loading", () => {
  it("loads the goosectl bridge dynamically from the main-window branch only", () => {
    expect(mainSource).not.toContain(
      'import { GoosectlBridge } from "@/features/goosectl"',
    );
    ...
    expect(sessionBranch).not.toContain("<OptionalGoosectlBridge />");
    expect(mainBranch).toContain("<OptionalGoosectlBridge />");
  });

  it("reports a failed dynamic bridge import without boot-failing the app", () => {
    expect(mainSource).toContain(
      'console.error("Failed to load goosectl bridge:"',
    );
    expect(mainSource).toContain(
  ...
  it("keeps AppShell's static graph away from goosectl bridge and command execution", () => {
    const graph = collectStaticValueImportGraph(appShellPath);

    for (const forbidden of [
      "features/goosectl/bridge/GoosectlBridge.tsx",
      "features/goosectl/bridge/lifecycle.ts",
      "features/goosectl/commands/registry.ts",
    ]) {
