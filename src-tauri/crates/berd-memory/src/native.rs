...
mod tests {
    ...
    #[test]
    fn memory_off_during_paused_open_blocks_topic_listing_and_recall() {
        for name in ["list_topics", "recall"] {
            let temp = tempfile::tempdir().unwrap();
            let root = temp.path().to_path_buf();
            let store = MemoryStore::with_key(&root, [17; 32]).unwrap();
            {
                let _lock = store.lock().unwrap();
                store
                    .commit_reviewed_document(
                        "topics/style.md",
                        "# Style\nSynthetic recall sentinel",
                        true,
                        None,
                    )
                    .unwrap();
            }
            berd_memory::store::write_policy(&root, true).unwrap();
            let read_root = root.clone();
            let (entered, waiting) = std::sync::mpsc::channel();
            let (release, resume) = std::sync::mpsc::channel();
            let reader = std::thread::spawn(move || {
                call_tool_with_opener(
                    &json!({"name": name, "arguments": {"topic": "Style"}}),
                    Some(read_root),
                    |_| {
                        entered.send(()).unwrap();
                        resume
            ...
            waiting
                ...
                .unwrap();
            let off = berd_memory::store::write_policy(&root, false);
            release.send(()).unwrap();
            let response = reader.join().unwrap();
            off.unwrap();
            assert_memory_blocked(&response);
            assert!(!response.to_string().contains("Synthetic recall sentinel"));
        }
    }
