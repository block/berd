...
//! goosectl`) and by a debug assertion in `tree::build_cli`. The artifacts
...
pub fn contract_errors(contract: &Contract) -> Vec<String> {
    ...
    for (noun, spec) in nouns {
        ...
        for (verb, verb_spec) in &spec.verbs {
            ...
            // stub's marker.
            let module = format!("`goosectl {noun} {verb}`'s command module");
            check_prose(
            ...
            let Some(action_spec) = contract.action(&spec.group, action) else {
                errors.push(format!(
                    "api-surface.json is missing `{}.{action}` (for `goosectl {noun} {verb}`)",
                    spec.group
                ));
                continue;
            };
            ...
            "session": {
                ...
                "verbs": {
                    "list": {
                        "action": "list",
                        "about": "List sessions",
                        "afterHelp": "Example: goosectl session list"
                    }
                }
            }
        }
