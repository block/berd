...
const APP_UPDATED: &str = "app updated — restart Goose";
const CONTROL_REMEDIATION: &str = "confirm Goose is running, app control is enabled, and this command is running inside a Goose-started agent session.";
...
pub fn ping(port: u16) -> Result<PingResponse, String> {
    ...
    if status != 200 {
        return Err(format!(
            "the listener on 127.0.0.1:{port} does not look like the Goose app \
...
pub fn handshake(lock_path: &Path) -> Result<Endpoint, Failure> {
    ...
    for attempt in 0..2 {
        ...
        let ping = ping(file.port).map_err(|detail| {
            Failure::env(format!(
                "the Goose desktop app is not reachable: {detail}. The app may have \
        ...
        if attempt == 0 {
            file = discovery::load(lock_path).map_err(|err| {
                Failure::env(format!(
                    "the Goose desktop app restarted its control endpoint and the new \
    ...
    Err(Failure::env(
        "the Goose desktop app's control endpoint restarted while goosectl was \
         connecting; retry the command, and confirm Goose is running, app control is \
         enabled, and this command is running inside a Goose-started agent session if \
         it keeps failing.",
    ))
}
...
fn transport_error_failure(port: u16, err: &ureq::Error) -> Failure {
    if is_connection_refused(err) {
        Failure::env(format!(
            "the Goose desktop app is not reachable on 127.0.0.1:{port} (connection \
