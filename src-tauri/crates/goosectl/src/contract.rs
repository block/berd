...
//! (`pnpm generate:goosectl-contract`); zod schemas and the command-module
...
pub struct Noun {
    ...
    pub group: String,
    /// One-line entry in `goosectl --help`'s command list.
    pub about: String,
...
pub struct Verb {
    ...
    pub action: String,
    /// One-line entry in `goosectl <noun> --help`'s verb list.
    pub about: String,
...
impl Contract {
    ...
    /// programmer error caught by `cargo test -p goosectl`, never a runtime
