//! goosectl — thin CLI client for the Goose desktop app's control broker.
//!
//! All command semantics (validation, defaults, side effects) live app-side
//! in the goosectl command registry; this binary only maps flags onto
...
//! (`pnpm generate:goosectl-contract`), which also author all help prose
...
/// (`goosectl … --json | head`) is routine CLI usage: a broken pipe counts
...
mod tests {
    ...
    fn contract_validates_cleanly() {
        ...
        assert!(
            ...
            "\nthe CLI contract is inconsistent:\n\n  - {}\n\n\
             ...
             then regenerate with `pnpm generate:goosectl-contract` \
             (see .agents/skills/goosectl-new-command/SKILL.md).\n",
            errors.join("\n  - ")
        );
    }
    ...
    fn doctor_is_a_normal_unknown_subcommand() {
        let err = try_parse(&["goosectl", "doctor"]).expect_err("doctor is not a command");
        ...
        for (noun, spec) in &surface_nouns() {
            ...
            for (verb, verb_spec) in spec.get("verbs").and_then(Value::as_object).unwrap() {
                ...
                let sub = cmd
                    ...
                    .unwrap_or_else(|| panic!("clap has `goosectl {noun} {verb}`"));
                for field in &fields {
                    ...
                    let arg = sub
                        ...
                        .unwrap_or_else(|| {
                            panic!("--{long} is missing on `goosectl {noun} {verb}`")
                        });
                    assert_eq!(
                        arg.is_required_set(),
                        required,
                        "--{long} requiredness on `goosectl {noun} {verb}` diverges from \
        ...
        for (noun, spec) in &surface_nouns() {
            ...
            for (verb, verb_spec) in spec.get("verbs").and_then(Value::as_object).unwrap() {
                let extra = minimal_invocation(noun, verb).unwrap_or_else(|| {
                    panic!("add a minimal invocation for `goosectl {noun} {verb}`")
                });
                let argv: Vec<&str> = [vec!["goosectl", noun, verb], extra].concat();
    ...
    /// Agents reach for plural nouns (`goosectl projects list`); the hidden
    ...
    fn plural_noun_aliases_parse_to_the_singular_commands() {
        for (plural, singular) in [
            ...
            assert_eq!(
                wire_of(&["goosectl", plural, "list"]),
                wire_of(&["goosectl", singular, "list"]),
    ...
    fn optional_flags_are_omitted_from_the_wire() {
        let (_, args) = wire_of(&["goosectl", "session", "create", "--prompt", "hi"]);
        assert_eq!(
            Value::Object(args),
    ...
    fn clear_project_maps_to_its_explicit_action() {
        let (command, args) =
            wire_of(&["goosectl", "session", "clear-project", "--session-id", "s"]);
        assert_eq!(command, "sessions");
        assert_eq!(
            Value::Object(args),
    ...
    fn timeout_ms_is_range_checked_client_side() {
        for out_of_range in ["999", "150001", "0"] {
            assert!(
                try_parse(&["goosectl", "session", "list", "--timeout-ms", out_of_range]).is_err(),
                "--timeout-ms {out_of_range} must be rejected"
            );
        }
        let matches = try_parse(&["goosectl", "session", "list", "--timeout-ms", "1000"])
    ...
    fn numeric_bounds_are_enforced_client_side() {
        for out_of_range in ["0", "101"] {
            assert!(
                try_parse(&["goosectl", "session", "list", "--limit", out_of_range]).is_err(),
    ...
    fn rendered_long_help(path: &[&str]) -> String {
        ...
        let mut bin_name = String::from("goosectl");
        for name in path {
            target = target
        ...
        // `goosectl …` invocation path instead of the bare leaf name.
    ...
    const EXPECTED_TOP_LEVEL_HELP: &str = r#"Control the Goose desktop app from the command line.

goosectl talks to the running Goose desktop app and acts on what the user
sees there:
...
Examples:
  ...
  goosectl session list --query "standup" --json
  goosectl session get --session-id <session-id> --messages 5 --json
...
Exit codes:
  ...
  2  transport failure between goosectl and the app — confirm Goose is running,
     app control is enabled, and this is a Goose-started agent session
  3  not running inside a Goose desktop app session, the app is not running,
     or this goosectl no longer matches the app — stderr explains which
"#;
...
Usage: goosectl session create [OPTIONS] --prompt <PROMPT>
...
Options:
      ...
      --harness-id <HARNESS_ID>
          Agent harness to run the session on (from `goosectl info harnesses`,
          e.g. "goose", "claude-acp", "codex-acp"). Defaults to the app default.

      --model-id <MODEL_ID>
          Id of the model to use (from `goosectl info models`).

      --agent-id <AGENT_ID>
          Id of the agent (persona) to use (from `goosectl agent list`).
...
Example:
  goosectl session create --prompt "Triage the failing nightly build" \
...
Result:
  ...
  progress later with `goosectl session get --session-id <session_id>`.
"#;

    const EXPECTED_SESSION_MOVE_HELP: &str = r#"Move a chat session into a project; the session list in the app regroups
immediately.

Usage: goosectl session move [OPTIONS] --session-id <SESSION_ID> --project-id <PROJECT_ID>
...
Example:
  goosectl session move --session-id <session-id> --project-id <project-id>
...
Usage: goosectl session clear-project [OPTIONS] --session-id <SESSION_ID>
...
Example:
  goosectl session clear-project --session-id <session-id>
...
"#;
    ...
    /// `cargo test -p goosectl dump_rendered_help -- --ignored`, review the
