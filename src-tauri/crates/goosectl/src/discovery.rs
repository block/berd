//! Discovery-file resolution: how goosectl finds the app's control endpoint.
...
/// `PROTOCOL_VERSION` in the `tauri-plugin-goosectl` crate
/// (src-tauri/plugins/goosectl) — the CLI does not depend on the plugin
...
pub const NOT_UNDER_APP: &str =
    "goosectl must run inside a Goose desktop app session (the app sets this up automatically)";

const REREAD_DELAY: Duration = Duration::from_millis(200);

/// Shape of the discovery file the goosectl broker writes on start
/// (`<app-data>/goosectl/control-<app-pid>.json`). Duplicated by hand from
/// the writer's struct in `tauri-plugin-goosectl`; keep in sync.
...
/// The lock path comes from `--lock-path` or `GOOSECTL_LOCK` (clap merges
...
pub fn load_with_retry(path: &Path) -> Result<DiscoveryFile, Failure> {
    load(path)
        ...
        .map_err(|err| {
            Failure::env(format!(
                "the Goose desktop app's control endpoint is unavailable ({err}); \
                 the app may not be running or app control may be disabled. \
                 confirm Goose is running, app control is enabled, and this \
                 command is running inside a Goose-started agent session."
            ))
        })
}
