//! Builds the goosectl clap Command tree at startup from the embedded
...
Control the Goose desktop app from the command line.
...
pub fn build_cli(contract: &Contract) -> Command {
    debug_assert!(
        ...
        "the embedded contract is inconsistent; run `cargo test -p goosectl` \
         for the full error list"
    );

    let mut cmd = Command::new("goosectl")
        .bin_name("goosectl")
        .term_width(HELP_WIDTH)
        .about("Control the Goose desktop app (sessions, projects, agents, skills) from the command line")
            ...
            Arg::new("lock_path")
                ...
                .help(
                    "Path to the app's control discovery file (internal plumbing; \
                     the app sets GOOSECTL_LOCK in every agent session it spawns)",
                ),
        )
...
fn noun_command(contract: &Contract, noun: &str, spec: &Noun) -> Command {
    let mut cmd = Command::new(noun.to_string())
        ...
        .subcommand_required(true);
    // Hidden plural aliases forgive `goosectl sessions list` etc. without
