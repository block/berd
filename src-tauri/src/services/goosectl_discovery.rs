//! Stale goosectl discovery-file cleanup.
//!
//! Each app instance's goosectl broker writes a discovery file at
//! `<app_data_dir>/goosectl/control-<app_pid>.json` and deletes it on
...
//! goosectl feature is off.

use std::path::Path;

use tauri_plugin_goosectl::{owner_pid_from_discovery_file_name, DISCOVERY_DIR_NAME};
...
pub fn sweep_stale_discovery_files(app_data_dir: &Path) {
    ...
    let entries = match std::fs::read_dir(&dir) {
        ...
        Err(error) => {
            log::warn!(
                "Failed to read goosectl discovery dir {}: {error}",
                dir.display()
            );
            return;
        }
    ...
    for entry in entries.flatten() {
        ...
        let path = entry.path();
        log::info!("Removing stale goosectl discovery file {}", path.display());
        if let Err(error) = std::fs::remove_file(&path) {
            log::warn!(
                "Failed to remove stale goosectl discovery file {}: {error}",
                path.display()
            );
        }
    }
