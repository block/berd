//! Loopback-only HTTP broker for the goosectl CLI.
//!
...
pub async fn start_server<D: CommandDispatcher>(
    ...
    tokio::spawn(async move {
        ...
        if let Err(err) = serve.await {
            log::error!("[goosectl] server error: {err}");
        }
    });
    Ok(ServerHandle {
        port,
...
fn forbidden_header_response<D>(ctx: &ServerContext<D>, headers: &HeaderMap) -> Option<Response> {
    ...
    violation.map(|message| {
        log::warn!("[goosectl] rejected request: {message}");
    ...
    match outcome {
        ...
        Err(err @ (BridgeError::Emit(_) | BridgeError::RendererDropped)) => {
            if let BridgeError::Emit(err) = &err {
                log::warn!("[goosectl] emit to main window failed: {err}");
            }
...
fn log_call(command: &str, result_code: &str, started: Instant) {
    ...
    log::info!(
        "[goosectl] /v1/call command={command} result={result_code} duration_ms={duration_ms}"
    );
}
...
mod tests {
    ...
    /// Invariant #1 of the goosectl architecture
    /// (docs/goosectl-architecture.md): no command-specific knowledge below
    ...
    fn broker_source_stays_free_of_command_literals() {
        ...
        let api: Value =
            serde_json::from_str(include_str!("../../../crates/goosectl/api-surface.json"))
            ...
            for name in &names {
                ...
                assert!(
                    ...
                    "broker {file} contains the command literal {literal}: the broker is \
                     transport-only (invariant #1, docs/goosectl-architecture.md); command \
