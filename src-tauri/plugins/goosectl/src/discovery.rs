//! Per-instance discovery ("lock") file the goosectl CLI reads to find the
...
/// file and echoed by `GET /v1/ping`. The goosectl CLI carries its own copy
/// (src-tauri/crates/goosectl); the CLI does not depend on this crate —
/// bump both together.
#[cfg_attr(not(feature = "server"), allow(dead_code))]
pub const PROTOCOL_VERSION: u32 = 1;

/// Directory under the app data dir holding the per-instance discovery files.
pub const DISCOVERY_DIR_NAME: &str = "goosectl";
...
/// `<app_data_dir>/goosectl/control-<pid>.json`. Per-instance (pid
...
pub(crate) fn remove_discovery_file(path: &Path) {
    if let Err(err) = std::fs::remove_file(path) {
        if err.kind() != std::io::ErrorKind::NotFound {
            log::warn!(
                "[goosectl] failed to remove discovery file {}: {err}",
                path.display()
            );
        }
    }
...
mod tests {
    ...
    const API_SURFACE: &str = include_str!("../../../crates/goosectl/api-surface.json");
    ...
    fn write_and_remove_lifecycle() {
        let base =
            std::env::temp_dir().join(format!("goosectl-discovery-test-{}", std::process::id()));
